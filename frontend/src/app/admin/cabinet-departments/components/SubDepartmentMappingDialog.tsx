...
import { cabinetSubDepartmentApi, medicalSupplySubDepartmentsApi } from "@/lib/api";
...
const MASTER_DEPARTMENTS_HREF = "/admin/management/departments";
...
export default function SubDepartmentMappingDialog({
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [linksRes, masterRes] = await Promise.all([
        cabinetSubDepartmentApi.getAll({ cabinetId: mapping.cabinet_id }),
        medicalSupplySubDepartmentsApi.getAll(),
      ]);
  ...
  const submitAdd = async () => {
    ...
    setSaving(true);
    try {
      const res = await cabinetSubDepartmentApi.create({
  ...
  const submitEdit = async (row: CabinetSubDepartmentLink) => {
    ...
    setSaving(true);
    try {
      const res = await cabinetSubDepartmentApi.update(row.id, {
        cabinet_id: mapping.cabinet_id,
  ...
  const doDelete = async () => {
    ...
    try {
      const res = await cabinetSubDepartmentApi.delete(deleteTarget.id);
      if (res.success) {
