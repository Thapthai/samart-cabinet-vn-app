...
import { EditStaffUserDialog } from '../EditStaffUserDialog';
import { StaffUsersSearchCard } from '../StaffUsersSearchCard';
import { StaffUsersTable } from '../StaffUsersTable';
import { RegeneratedCredentialsDialog } from '../RegeneratedCredentialsDialog';
import { StaffUserDepartmentDialog } from '../StaffUserDepartmentDialog';
...
export default function StaffUsersTab() {
  ...
  const [deptDialogUser, setDeptDialogUser] = useState<StaffUser | null>(null);
  ...
  const fetchStaffUsers = async () => {
    try {
      setLoading(true);
      const response = (await staffUserApi.getAllStaffUsers()) as {
        success?: boolean;
        data?: StaffUser[];
        message?: string;
      };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('ต้องการปิดการใช้งาน Staff User นี้หรือไม่? (บัญชียังอยู่ในระบบแต่ใช้งานไม่ได้)'))
      return;
    try {
      const response = (await staffUserApi.deleteStaffUser(id)) as {
        success?: boolean;
        message?: string;
      };
      if (response?.success) {
  ...
  const handleRegenerateSecret = async (id: number) => {
    if (!confirm('คุณแน่ใจหรือไม่ว่าต้องการสร้าง Client Secret ใหม่? Secret เก่าจะใช้งานไม่ได้อีก'))
      return;
    try {
      ...
      if (response?.success && response?.data) {
        ...
        toast.error(
          (response as { message?: string }).message || 'ไม่สามารถสร้าง Client Secret ใหม่ได้',
        );
      }
    } catch (error: unknown) {
      const err = error as { message?: string };
  ...
  const openDepartmentDialog = (staff: StaffUser) => {
    setDeptDialogUser(staff);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900">จัดการ Staff Users</h2>
            <p className="text-sm text-gray-500 mt-1">
              จัดการบัญชี Staff และ Client Credentials สำหรับ API
            </p>
          </div>
        </div>
      </div>
      ...
      <StaffUsersTable
        ...
        onManageDepartments={openDepartmentDialog}
      ...
      <StaffUserDepartmentDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) setDeptDialogUser(null);
        }}
        user={deptDialogUser}
      />
