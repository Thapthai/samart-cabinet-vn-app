...
export default function SearchableSelect({
  ...
  const useFloatingLayout = positionMode === "floating" || portalTargetRef != null;
  ...
  useLayoutEffect(() => {
    ...
    const run = () => {
      if (cancelled) return;
      if (positionMode !== "floating" && portalTargetRef != null && !portalTargetRef.current) {
        attempts += 1;
        if (attempts < 30) {
    ...
    const portalScrollEl =
      portalTargetRef != null && positionMode !== "floating" ? portalTargetRef.current : null;
  ...
  useEffect(() => {
    if (!isOpen) initialLoadDone.current = false;
  }, [isOpen]);
  ...
  useEffect(() => {
    ...
    const debounce = setTimeout(() => search(searchTerm), 300);
  ...
  if (selectedOption) lastSelectedRef.current = selectedOption;
  else if (trimmedValue && (initialDisplay?.label || initialDisplay?.subLabel)) {
  ...
  const pinnedSelectedOption: Option | null =
    !q && trimmedValue && !filteredOptions.some((o) => o.value === trimmedValue)
      ? selectedOption ??
        lastSelectedRef.current ??
  ...
  return (
    <div className="min-w-0 w-full space-y-2" ref={dropdownRef}>
      ...
      <div className={cn("relative", showInlinePanel && "z-[10000]")}>
        <button
          ref={triggerRef}
          type="button"
          onClick={() => {
