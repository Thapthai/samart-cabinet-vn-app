...
export default function StaffItemMasterManagementPage() {
  ...
  return (
    <>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger className={cn('w-[140px]', fieldInputClass)}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => handlePageChange(1)} disabled={page === 1 || loading}>
                    แรกสุด
                  </Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => handlePageChange(page - 1)} disabled={page === 1 || loading}>
                    ก่อนหน้า
                  </Button>
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => handlePageChange(page + 1)} disabled={page === totalPages || loading}>
                    ถัดไป
                  </Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => handlePageChange(totalPages)} disabled={page === totalPages || loading}>
                    สุดท้าย
                  </Button>
                </div>
              </div>
      ...
      <CreateItemDialog open={createOpen} onOpenChange={setCreateOpen} onSuccess={fetchList} />
      <EditItemDialog open={editOpen} onOpenChange={setEditOpen} item={selected} onSuccess={fetchList} />
      <DeleteItemDialog open={deleteOpen} onOpenChange={setDeleteOpen} item={selected} onSuccess={fetchList} />
    </>
  );
}
