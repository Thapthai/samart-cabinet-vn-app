...
import PrintStickerFilterSection from './components/PrintStickerFilterSection';
...
import { AUTO_FETCH_LIMIT, MAX_PRINT, MAX_TOTAL_LABELS, PAGE_SIZE } from './constants';
...
export default function StaffPrintStickerPage() {
  ...
  const fetchCabinetItems = useCallback(async () => {
    ...
    try {
      ...
      setSelectedLines(
        need.map((row) =>
          buildLineFromRow(row, Math.max(1, Number(row.refill_qty ?? 0)), Math.max(0, Number(row.refill_qty ?? 0))),
        ),
      );
    } catch {
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">เฉพาะ Division และตู้ตามสิทธิ์ role</p>
        </div>
      </div>
