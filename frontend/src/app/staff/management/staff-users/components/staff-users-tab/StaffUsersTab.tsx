...
import {
  ...
  type CreateStaffUserFormData,
  type StaffRoleOption,
  type StaffUser,
} from '@/app/staff/management/permission-users/types';
import CreateStaffUserDialog from '@/app/staff/management/permission-users/components/CreateStaffUserDialog';
import EditStaffUserDialog, {
  type EditStaffUserFormData,
} from '@/app/staff/management/permission-users/components/EditStaffUserDialog';
...
export default function StaffUsersTab() {
  ...
  const handleManageDepartments = (user: StaffUser) => {
    if (
      !staffPortalCanManageStaffUserRow(
        viewerRoleCode,
        readStaffUserIdFromStorage(),
      ...
      toast.error('ไม่มีสิทธิ์จัดการผู้ใช้รายนี้');
      return;
    }
    setDeptDialogUser(user);
