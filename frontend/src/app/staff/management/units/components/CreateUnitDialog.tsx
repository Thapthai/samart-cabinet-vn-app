...
export default function CreateUnitDialog({ open, onOpenChange, onSuccess }: CreateUnitDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await staffUnitsApi.create(payload);
      if (res.success) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            บันทึกผ่าน API Staff — ชื่อหน่วยใช้ผูกกับรายการ Item ตามสิทธิ์ของคุณ
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="staff-create-unit-name">ชื่อหน่วย (UnitName)</Label>
            <Input
              id="staff-create-unit-name"
              value={unitName}
