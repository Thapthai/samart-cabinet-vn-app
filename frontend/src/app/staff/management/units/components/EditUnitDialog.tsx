...
export default function EditUnitDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await staffUnitsApi.update(unit.id, payload);
      if (res.success) {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            แก้ไขหน่วยนับ (Staff) {unit ? `#${unit.id}` : ''}
          </DialogTitle>
          <DialogDescription>แก้ไขชื่อหน่วยและ B_ID หรือคืนสถานะใช้งาน — ผ่าน API Staff</DialogDescription>
        </DialogHeader>
        {unit && (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="staff-edit-unit-name">ชื่อหน่วย</Label>
              <Input
                id="staff-edit-unit-name"
                value={unitName}
            ...
            {unit.isCancel && (
              <div className="flex items-center space-x-2 rounded-md border p-3">
                <Checkbox
                  id="staff-restore-unit"
                  checked={restoreActive}
                  onCheckedChange={(c) => setRestoreActive(c === true)}
                />
                <Label htmlFor="staff-restore-unit" className="text-sm font-normal cursor-pointer">
