...
export type UnitStatusFilter = 'all' | 'active' | 'cancelled';

export interface UnitsSearchCardProps {
  keywordInput: string;
...
export default function UnitsSearchCard({
  keywordInput,
  activeKeyword,
  statusFilter,
      ...
      <CardContent>
        <div className="mb-4 flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-slate-500">ค้นจากชื่อหน่วย (UnitName)</p>
          </div>
        </div>

        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_160px] lg:items-end">
          <div className="space-y-1.5 sm:col-span-2 lg:col-span-1">
            <label htmlFor="staff-unit-keyword" className="text-xs font-medium text-slate-600">
              คำค้นหา
            </label>
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
              <Input
                id="staff-unit-keyword"
                placeholder="เช่น ชื่อหน่วยนับ..."
                value={keywordInput}
          ...
          <div className="space-y-1.5">
            <label htmlFor="staff-unit-status-filter" className="text-xs font-medium text-slate-600">
              สถานะ
            </label>
            <Select value={statusFilter} onValueChange={(v) => onStatusFilterChange(v as UnitStatusFilter)}>
              <SelectTrigger id="staff-unit-status-filter" className={cn('h-10 w-full shadow-sm', fieldInputClass)}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        {hasActiveFilters ? (
          <div className="mt-4 flex flex-wrap items-center justify-end gap-2 border-t border-slate-200/70 pt-4">
            ...
            {statusFilter !== 'all' ? (
              ...
              >
                {statusFilter === 'active' ? 'ใช้งาน' : 'ยกเลิกแล้ว'}
              </span>
            ) : null}
            <Button
              type="button"
